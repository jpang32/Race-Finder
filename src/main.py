...
def _get_crawl_delay(base_url: str) -> int:
	...
	if crawl_delay_search is None or not crawl_delay_search.group(1):
		logging.log(
			logging.DEBUG, f"Crawl Delay not found in robots.txt. Setting to default value of {DEFAULT_CRAWL_DELAY})"
		)
...
def parse_args():
	...
	parser.add_argument(
		...
		"--race-type",
		type=str,
		help="Type of race that you are interested in running.",
		choices=[race_type.value for race_type in RaceType],
		required=True,
	)
...
def main():
	...
	# path_base = "/race/list/ny/upcoming"
	# path_params = ""
	#
	...
	# For each row, grab each the first 3 cells
	# How to iterate through the pages

	# If user
	# Filter on the date column first,
